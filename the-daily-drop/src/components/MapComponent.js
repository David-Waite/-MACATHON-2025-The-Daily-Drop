...
function MapComponent({
  ...
  const [renderedPosition, setRenderedPosition] = useState(userPosition); // State for the marker's CURRENT animated position // Refs for animation state to avoid unnecessary effect triggers
  ...
  const animationTargetPosition = useRef(null); // --- Map Load/Unload Callbacks ---
  ...
  const onUnmount = useCallback(() => {
    setMap(null); // Cancel any ongoing animation when map unmounts
  ...
  const onUserMarkerLoad = useCallback(
    (marker) => {
      ...
      setUserMarkerInstance(marker); // Initialize rendered position when marker loads if not already set

      setRenderedPosition((prev) => prev ?? userPosition);
    },

    [userPosition]
  ); // Dependency needed if userPosition is available on first load // --- Animation Effect ---
  ...
  useEffect(() => {
    ...
    if (!userMarkerInstance || !userPosition) {
      ...
      } // Optionally set renderedPosition to null or keep last known? // setRenderedPosition(userPosition); // Sync if target becomes null

      return;
    } // Update target position ref

    animationTargetPosition.current = userPosition; // Set the start position for the animation // If no animation is running, start from the current rendered position

    if (!animationFrameId.current) {
      animationStartPosition.current = renderedPosition ?? userPosition; // Use current rendered or target if null // Only start a new animation if the target is different enough from start // (Avoids tiny animations for minimal changes) - Optional threshold check can be added here
    ...
    } // If an animation IS already running, the loop will pick up the new target position naturally // Cleanup function for this effect run (doesn't cancel animation here)
  ...
  }, [userPosition, userMarkerInstance, renderedPosition]); // renderedPosition needed to define start point if animation isn't running // Separate function to request and manage the animation frame loop
  ...
  const animationLoop = (timestamp) => {
    ...
    const progress = Math.min(1, elapsed / ANIMATION_DURATION_MS); // Ensure progress doesn't exceed 1 // Interpolate latitude and longitude
    ...
    const newRenderedPos = { lat: currentLat, lng: currentLng }; // Directly update the marker's position on the map

    userMarkerInstance.setPosition(newRenderedPos); // Update React state for the rendered position (important for next animation start point)
    ...
    if (progress < 1) {
      ...
      animationStartPosition.current = null; // Target ref remains for next update comparison
    }
  }; // --- Render Logic ---

  if (loadError) return <div>Error loading map: {loadError.message}</div>;

  if (!isLoaded) return <div>Loading Map...</div>; // Map center is controlled by parent now

  const mapCenter = center || defaultCenter; // User Location Icon (Constant Size Blue Dot)
  ...
  return (
    ...
    >
      ...
      {(userPosition || renderedPosition) && (
        <MarkerF // Use renderedPosition for display, but don't list it as a dependency for onLoad
          position={renderedPosition ?? userPosition} // Use current animated position, fallback to target
          title={"You are here"}
          icon={userLocationIcon}
          zIndex={5}
          onLoad={onUserMarkerLoad} // <-- Get the marker instance // Optimization: prevent marker re-renders just due to position prop changing // The animation loop updates the instance directly. // However, we DO need it to render initially at the right spot. // Keying it might cause remounts, which we don't want. // Let's rely on `setPosition` in the loop.
        />
      )}
      {/* Drop Markers */}
      {drops.map((drop) => (
